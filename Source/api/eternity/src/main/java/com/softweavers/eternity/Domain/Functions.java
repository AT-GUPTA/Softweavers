...
//            }
//            BigDecimal result = BigDecimal.valueOf(Math.PI).divide(loop_result, MATH_CONTEXT);
//            return result;
//        }
//    }
...
//    }
    ...
    public BigDecimal pow(BigDecimal base, BigDecimal exp) {
        ...
        if (exp.compareTo(BigDecimal.ONE) >= 0) {
            temp = pow(base, exp.divide(BigDecimal.TWO, MATH_CONTEXT));
...
//    }
    ...
    public BigDecimal gamma(BigDecimal z) {
        ...
        if (z.compareTo(new BigDecimal(0.5)) == -1) {
            ...
            y = new BigDecimal(Math.PI).divide(sin(a)).multiply(gamma(b), MATH_CONTEXT);
        } else {
            z = z.subtract(new BigDecimal(1));
            BigDecimal x = new BigDecimal("0.99999999999980993");
            for (int i = 0; i < lanczos.length; ++i) {
                x = x.add(lanczos[i].divide(z.add(new BigDecimal(i + 1)), MATH_CONTEXT));
            }
    ...
    public BigDecimal standardDeviation(String[] input){
